      ...
      (item: LogoItem, key: React.Key) => {
        if (renderItem) {
          return (
            <li
              className={cx(
                'flex-none inline-flex items-center justify-center text-[length:var(--logoloop-logoHeight)] leading-[1]',
                isVertical ? 'mb-[var(--logoloop-gap)]' : 'mr-[var(--logoloop-gap)]',
                scaleOnHover && 'overflow-visible group/item'
              )}
              style={isVertical ? undefined : { height: 'var(--logoloop-logoHeight)' }}
              key={key}
              role="listitem"
            >
              {renderItem(item, key)}
