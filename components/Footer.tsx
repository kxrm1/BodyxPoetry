  ...
  return (
    <footer className="relative w-full bg-[#1C2716] text-[#FAF7F2]/80 py-16 px-6 sm:px-10 lg:px-16 border-t border-[#FAF7F2]/10 font-serif select-none">
      ...
      <div className="max-w-6xl mx-auto relative z-20 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        {/* Links */}
        <div className="flex flex-wrap items-center justify-center gap-8 text-xs uppercase tracking-[0.2em] text-[#FAF7F2]/70">
          <a
            href="https://instagram.com"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-[#D4C5A9] transition-colors duration-200"
          >
            Instagram
          </a>
          <a
        ...
        <div className="flex items-center gap-6 text-xs text-[#FAF7F2]/50">
          <span>© {new Date().getFullYear()} Body × Poetry</span>
          <button
            type="button"
            onClick={scrollToTop}
            aria-label="Back to top"
            className="group flex items-center gap-1.5 hover:text-[#D4C5A9] transition-colors duration-200 cursor-pointer"
          >
            <span className="uppercase tracking-widest text-[10px]">Top</span>
            <ArrowUp
              weight="light"
              className="w-3.5 h-3.5 text-[#D4C5A9] group-hover:-translate-y-0.5 transition-transform duration-200"
            />
          </button>
        </div>
      </div>
